...
export type UserFields = Pick<
  RavenUser,
  'name' | 'full_name' | 'user_image' | 'first_name' | 'enabled' | 'type' | 'availability_status' | 'custom_status'
>
...
export const UserListProvider = ({ children }: PropsWithChildren) => {
  ...
  if (usersError) {
    return (
      <Flex align='center' justify='center' px='4' mx='auto' className='w-[50vw] h-screen'>
  ...
  return <UserListContext.Provider value={{ users, enabledUsers }}>{children}</UserListContext.Provider>
}
