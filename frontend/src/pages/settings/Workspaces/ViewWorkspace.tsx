...
const ViewWorkspaceContent = ({ data, mutate }: { data: RavenWorkspace; mutate: SWRResponse['mutate'] }) => {
  ...
  const onSubmit = (data: RavenWorkspace) => {
    updateDoc('Raven Workspace', data.name, data).then((doc) => {
      toast.success('Saved')
      methods.reset(doc)
        ...
        <SettingsContentContainer>
          <SettingsPageHeader
            ...
            breadcrumbs={[
              ...
              { label: data.workspace_name, href: '', copyToClipboard: true }
            ]}
          />
          <ErrorBanner error={error} />
          {data && (
            <Tabs.Root defaultValue='details'>
              <Tabs.List>
                <Tabs.Trigger value='details'>
                  <LuDock {...ICON_PROPS} /> Details
                </Tabs.Trigger>
                <Tabs.Trigger value='members'>
                  <LuUsers {...ICON_PROPS} /> Members
                </Tabs.Trigger>
              </Tabs.List>
              <Box pt='4'>
