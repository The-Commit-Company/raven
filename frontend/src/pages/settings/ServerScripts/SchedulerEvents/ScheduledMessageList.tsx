...
export const List = ({ data }: { data: RavenSchedulerEvent[] }) => {
  return (
    <Flex direction='column' gap='4' width='100%' className='animate-fadein'>
      {data?.map((item, index) => <ScheduledMessageItem item={item} key={index} />)}
    </Flex>
  )
}
...
const ScheduledMessageItem = ({ item }: { item: RavenSchedulerEvent }) => {
  ...
  return (
    <Flex
      ...
      width='100%'
      justify={'between'}
      className='border border-gray-4 dark:border-gray-6 rounded-radius2 p-3'
    >
      <Flex direction='row' align='center' justify='between'>
        <Flex direction='column' gap='1'>
          ...
          >
            Created by {item.owner} on <DateMonthYear date={item.creation} />
          </Text>
        </Flex>
