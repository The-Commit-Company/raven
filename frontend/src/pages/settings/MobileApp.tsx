  ...
  return (
    <PageContainer>
      <SettingsContentContainer>
        <SettingsPageHeader title={__('Mobile App')} description={__('Do more with Raven on your mobile.')} />
        {!isRavenAdmin && (
          <CustomCallout
        ...
        <Text size='2'>
          Raven uses OAuth to authenticate users securely on the mobile app.
          <br />
          To use the Raven mobile app, you need to first create an{' '}
          <Link href='https://docs.frappe.io/framework/user/en/using_frappe_as_oauth_service' target='_blank'>
            OAuth Client <FiExternalLink />
          </Link>
          .
          <br />
          <br />
          <Text as='span' weight='medium'>
            This is a one-time single click setup.
          </Text>{' '}
          Click the button below to configure the OAuth Client.
        </Text>
        <div className='flex gap-2'>
          {ravenSettings?.oauth_client ? (
            <Button onClick={createOAuthClient} className='not-cal'>
              Reset OAuth Client
            </Button>
          ) : (
            <Button onClick={createOAuthClient} disabled={loading} className='not-cal'>
          ...
          {ravenSettings?.oauth_client && (
            <Button asChild color='gray' variant='outline' className='not-cal'>
              <Link color='gray' target='_blank' href={`/app/oauth-client/${ravenSettings.oauth_client}`}>
                View OAuth Client
                <FiExternalLink />
              </Link>
            </Button>
