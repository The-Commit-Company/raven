...
const FileSourcesList = () => {
  const isRavenAdmin = hasRavenAdminRole() || isSystemManager()

  const { data, isLoading, error, mutate } = useFrappeGetDocList<RavenAIFileSource>(
      ...
      <SettingsContentContainer>
        ...
        {(data?.length === 0 || !isRavenAdmin) && (
          <EmptyState>
            ...
            <EmptyStateDescription>
              AI Agents can use files as data sources to get more context, read instructions and execute tasks. You can
              upload files here and use them across multiple agents.
            </EmptyStateDescription>
            {isRavenAdmin && (
              <Button asChild className='not-cal'>
                Upload a file
              </Button>
            )}
          </EmptyState>
  ...
  return (
    <>
      ...
      <AlertDialog.Root open={!!selected} onOpenChange={onOpenChange}>
        <AlertDialog.Content>
          <AlertDialog.Title>Xóa file</AlertDialog.Title>
          <AlertDialog.Description>
            Bạn có chắc muốn xóa file <strong>{selected?.file_name}</strong>?
          </AlertDialog.Description>
          <Stack gap='2' pt='4'>
