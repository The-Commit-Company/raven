...
import { FrappeConfig, FrappeContext, SWRResponse, useFrappeGetDoc, useFrappeUpdateDoc } from 'frappe-react-sdk'
...
const ViewBotContent = ({ data, mutate }: { data: RavenBot; mutate: SWRResponse['mutate'] }) => {
  ...
  const onSubmit = (data: RavenBot) => {
    updateDoc('Raven Bot', data.name, data).then((doc) => {
      toast.success('Saved')
      methods.reset(doc)
      ...
      <FormProvider {...methods}>
        <SettingsContentContainer>
          <SettingsPageHeader
            ...
            breadcrumbs={[
              { label: 'Agents', href: '../' },
              { label: data.name, href: '', copyToClipboard: true }
            ]}
          />
          <ErrorBanner error={error} />
          <BotForm isEdit={true} />
...
const OpenChatButton = ({ bot }: { bot: RavenBot }) => {
  ...
  return (
    <Button variant='surface' color='gray' type='button' className='not-cal' onClick={openChat}>
      Open Chat
      <FiExternalLink />
    </Button>
  )
