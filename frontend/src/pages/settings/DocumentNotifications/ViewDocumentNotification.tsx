...
const ViewDocumentNotificationContent = ({
  data,
  mutate
}: {
  data: RavenDocumentNotification
  ...
  const onSubmit = (data: RavenDocumentNotification) => {
    updateDoc('Raven Document Notification', data.name, data).then((doc) => {
      toast.success('Saved')
      methods.reset(doc)
      ...
      <FormProvider {...methods}>
        <SettingsContentContainer>
          <SettingsPageHeader
            ...
            breadcrumbs={[
              { label: 'Document Notifications', href: '../' },
