...
const ErrorPage = () => {
  ...
  return (
    <Flex className='h-screen w-screen bg-gray-2' align='center' justify='center'>
      <Stack gap='4'>
        <Heading as='h1' size='5' className='text-center not-cal'>
          {errorDueToUpdate ? 'A new update is available.' : 'There was an unexpected error.'}
        </Heading>
        <Text>
          If you face this error again, please report it either on{' '}
          <Link target='_blank' href='https://github.com/The-Commit-Company/raven/issues'>
            GitHub
          </Link>{' '}
          or{' '}
          <Link target='_blank' href='https://support.ravenchat.ai/'>
            {' '}
            our support portal
          </Link>
          .
        ...
        {!errorDueToUpdate && (
          <details>
            <summary>
              <Text size='2'>Show error details</Text>
            </summary>
            <Code color='gray'>{(error as Error).message}</Code>
          </details>
        )}
        <HStack justify='center'>
          <Button size='2' className='not-cal' onClick={reloadPage}>
            {errorDueToUpdate ? 'Upgrade to a better experience' : 'Reload the Page'}
          </Button>
          {!errorDueToUpdate && (
            <Button variant='soft' color='gray' size='2' className='not-cal' onClick={goToChannels}>
              Back to Channels
            </Button>
          )}
        </HStack>
