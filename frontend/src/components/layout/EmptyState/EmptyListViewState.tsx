...
export const EmptyStateIcon = ({ children }: PropsWithChildren) => {
  return <Slot className='text-5xl text-gray-10'>{children}</Slot>
}

export const EmptyStateTitle = ({ children }: PropsWithChildren) => {
  return (
    <Text as='span' size='5' className='mt-4 text-center text-gray-12 font-semibold'>
      {children}
    </Text>
  )
}
...
export const EmptyStateDescription = ({ children }: PropsWithChildren) => {
  return (
    <Text as='span' size='2' className='mt-3 mb-4 max-w-lg text-center text-gray-11'>
      {children}
    </Text>
  )
}

export const EmptyStateLinkAction = ({
  children,
  buttonProps,
  ...props
}: LinkProps & PropsWithChildren<{ buttonProps?: ButtonProps }>) => {
  return (
    <Button asChild className='not-cal' {...buttonProps}>
