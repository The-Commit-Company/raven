...
export const SidebarGroupLabel = ({ children, ...props }: SidebarGroupLabelProps) => {
  return (
    <Text
      ...
      weight='bold'
      {...props}
      className={clsx('cal-sans text-gray-12 dark:text-gray-300', props.className)}
    >
      {children}
    </Text>
  )
...
export const SidebarGroupList = ({ children, ...props }: SidebarGroupListProps) => {
  return (
    <Flex
      direction='column'
      {...props}
      className={clsx(`gap-0.5 transition-all ease-ease-out-cubic duration-200 overflow-hidden`, props.className)}
    >
      {children}
    </Flex>
  )
      ...
      {({ isActive }) => {
        return (
          <Flex
            ...
            px='2'
            className={clsx(
              'cursor-pointer text-gray-12 user-select-none rounded-md no-underline sm:hover:bg-gray-3 active:bg-gray-3',
...
export const SidebarViewMoreButton = ({ expanded, onClick, ...props }: SidebarViewMoreButtonProps) => {
  return (
    <IconButton
      ...
      {...props}
      className={clsx(
        'cursor-pointer transition-all text-gray-10 bg-transparent sm:hover:bg-gray-3 ease-ease group-hover:text-gray-12',
        props.className
      )}
    >
...
export const SidebarBadge = ({ children, className, ...props }: BadgeProps) => {
  return (
    <Theme accentColor='gray'>
      <div
        className={clsx(
          `flex items-center justify-center min-w-2 text-accent-a11 dark:text-accent-a11 dark:bg-accent-a3 bg-accent-a4 text-xs py-0.5 px-2 rounded-radius2
