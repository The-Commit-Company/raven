    ...
    <Popover.Root onOpenChange={onClose}>
      <Popover.Trigger>
        ...
        >
          ...
          {mentionsCount && mentionsCount?.message > 0 && (
            <Box className='rounded-full absolute -right-0.5 -bottom-0.5 sm:right-1.5 sm:bottom-0 bg-red-11 dark:bg-red-9 text-white w-4 h-4 flex items-center justify-center'>
              <Text as='span' size='1'>
                {mentionsCount?.message}
              </Text>
            </Box>
          )}
        </IconButton>
              ...
              {mention.is_thread ? (
                ...
                <>
                  <HStack align='center' className='inline-flex gap-0.5'>
                    in <ChannelIcon type={mention.channel_type} size='14' className='mt-[0.5px]' />{' '}
...
const TimeStamp = ({ creation }: { creation: string }) => {
  return (
    <Text as='span' size='1' className='text-gray-11 shrink-0'>
