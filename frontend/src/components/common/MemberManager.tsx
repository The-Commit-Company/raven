...
const MemberManager = ({ currentMembers, onChange }: Props) => {
  ...
  const userMembershipMap: Record<string, { is_admin: boolean; is_member: boolean }> = currentMembers.reduce(
    (acc, member) => {
      acc[member.user] = {
        ...
        <TableVirtuoso
          ...
          itemContent={(index, user) => {
            return (
              <MemberRow
                member={user}
                membership={userMembershipMap[user.name]}
                key={user.name}
...
const MemberRow = ({ member, onMemberChange, membership }: MemberRowProps) => {
  return (
    <>
      <Table.Cell
        className='w-[50%]'
        onClick={() => onMemberChange(member.name, !membership?.is_member, membership?.is_admin)}
      >
