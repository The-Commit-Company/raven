...
export const ArchiveChannelModal = ({
  onClose,
  onCloseViewDetails,
  channelData,
  isDrawer
}: ArchiveChannelModalProps) => {
  ...
  return (
    <>
      {isDrawer ? <Title>Archive this channel? </Title> : <AlertDialog.Title>Archive this channel? </AlertDialog.Title>}

      <Flex direction='column' gap='4'>
        <ErrorBanner error={error} />
        <Text size='2'>
          Please understand that when you archive <strong>{channelData?.channel_name}</strong>:
        </Text>
        <Flex direction='column'>
          <ul className={'list-inside'}>
            <li>
              <Text size='2'>It will be removed from your channel list</Text>
            </li>
            <li>
              <Text size='2'>No one will be able to send messages to this channel</Text>
            </li>
          </ul>
        </Flex>
        <Text size='2'>
          You will still be able to find the channel’s contents via search. And you can always unarchive the channel in
