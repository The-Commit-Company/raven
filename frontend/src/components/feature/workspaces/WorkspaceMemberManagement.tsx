...
export const useFetchWorkspaceMembers = (workspaceID: string) => {
  return useFrappeGetCall<{ message: WorkspaceMemberFields[] }>(
    ...
    <>
      <Table.Cell>
        <HStack align='center'>
          ...
          <Text as='span'>{user?.full_name ?? member.user}</Text>
          {member.is_admin ? (
      ...
      {isAdmin ? (
        <Table.Cell align='center'>
          <MemberActions member={member} workspaceID={workspaceID} />
        </Table.Cell>
      ) : null}
    </>
  )
    ...
    <div className='flex items-center gap-2 justify-center h-full'>
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content className='min-w-36'>
          {member.is_admin ? (
            <RemoveAdminButton memberID={member.name} onUpdate={onUpdate} />
          ) : (
            <MakeAdminButton memberID={member.name} onUpdate={onUpdate} />
          )}
...
const RemoveMemberButton = ({ memberID, onUpdate }: { memberID: string; onUpdate: () => void }) => {
  ...
  return (
    <DropdownMenu.Item onClick={removeMember} color='red'>
...
const MakeAdminButton = ({ memberID, onUpdate }: { memberID: string; onUpdate: () => void }) => {
  ...
  return (
    <DropdownMenu.Item onClick={makeAdmin}>
      <BiCrown fontSize={16} />
      Make Admin
    </DropdownMenu.Item>
  )
...
const RemoveAdminButton = ({ memberID, onUpdate }: { memberID: string; onUpdate: () => void }) => {
  ...
  return (
    <DropdownMenu.Item onClick={removeAdmin}>
      <BiCrown fontSize={16} />
      Remove Admin
    </DropdownMenu.Item>
  )
...
const ManageMembersDialog = ({ workspaceID }: { workspaceID: string }) => {
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={setOpen}>
      <Dialog.Trigger>
        <Button className='not-cal' color='gray' size='2' variant='soft' type='button'>
          <BiCog fontSize={16} /> Manage Members
        </Button>
      </Dialog.Trigger>
...
const ManageMembersDialogContent = ({
  workspaceID,
  onClose,
  members
}: {
