    ...
    <Tabs.Root defaultValue='general'>
      <Tabs.List>
        <Tabs.Trigger value='general'>
          <LuBell {...ICON_PROPS} /> Details
        </Tabs.Trigger>
        <Tabs.Trigger value='recipients'>
          <LuUsers {...ICON_PROPS} /> Recipients
        </Tabs.Trigger>
        <Tabs.Trigger value='condition'>
          <LuWorkflow {...ICON_PROPS} /> Conditions
        </Tabs.Trigger>
      </Tabs.List>
      <Box pt='4'>
...
const GeneralTab = ({ isEdit }: { isEdit: boolean }) => {
  const {
    ...
    control,
    formState: { errors }
  } = useFormContext<RavenDocumentNotification>()
  ...
  return (
    <Stack gap='4'>
      <Grid columns={'2'} gap='4' align='center'>
        <Stack>
          <Box>
            <Label htmlFor='notification_name' isRequired>
              Name
            </Label>
            <TextField.Root
        ...
        <Stack>
          <Box>
            <Label htmlFor='send_alert_on' isRequired>
              Send Alert On
            </Label>
            <Controller
              control={control}
        ...
        <Stack>
          ...
          />
          <HelperText>The document type to send the notification for.</HelperText>
      ...
      <Stack>
        ...
        <HelperText size='2'>
          ...
          <br />
          You can use data from the document like this: <Code>{`{{ doc.employee_name }}`}</Code>. You can also use
          functions like <Code>{`{{ frappe.utils.today() }}`}</Code>.
          <br />
          For a complete list of functions, see{' '}
          <Link href='https://docs.frappe.io/framework/user/en/api/jinja'>Frappe's Jinja documentation</Link>.
        </HelperText>
      </Stack>
...
const DoctypeVariables = ({ doctype, withoutJinja }: { doctype: string; withoutJinja?: boolean }) => {
  ...
  const fields = useMemo(() => {
    ...
    return doc.fields
      ?.filter(
        (field) =>
          in_list(VALID_FIELD_TYPES, field.fieldtype) &&
          field.fieldname &&
  ...
  return (
    <Stack>
      <Text size='2'>Here are some variables you can use. Simply copy by clicking on the variable.</Text>
      <Box>
        <VisuallyHidden>
  ...
  return (
    <Stack gap='4'>
      <HStack justify={'between'} align={'center'}>
        <Text size='2'>
          Recipients are the users who will receive the notification. You can choose to send the notification to
          channels or specific users.
        </Text>
        <Button onClick={onAddRow} variant='soft' size={'2'} type='button' className='not-cal'>
          Add Recipient
        </Button>
      </HStack>
      ...
      <Text size='2'>
        ...
        <br />
        For example, you might have the employee ID in the document and you want to send the notification to the user
        linked to that employee ID. You can use the following variable to get the recipient:{' '}
        <Code color='gray' size='2'>{`{{ frappe.db.get_value('Employee', doc.employee_id, 'user') }}`}</Code>
      </Text>
    </Stack>
  )
}
      ...
      <Box>
        <Flex direction={'row'} gap={'4'} align={'start'}>
          ...
          >
            ...
            <TextArea id='condition' {...register('condition')} placeholder='e.g. doc.docstatus == 1' rows={10} />
            ...
            >
              Optional: The notification will be sent if this expression is true.
            </HelperText>
          </Flex>
          <Code color='gray' size='2' className='mt-7 p-4'>
            ...
            <br />
            <br />
            doc.status == "Received" or doc.status == "Partially Received" <br />
            <br />
            {`doc.total > 10000`}
          </Code>
        </Flex>
