    ...
    <DropdownMenu.Sub>
      <DropdownMenu.SubTrigger>
        <Flex gap={'2'} align='center'>
          {getStatusText(myProfile?.availability_status ?? '')}
        </Flex>
      </DropdownMenu.SubTrigger>
...
export const getStatusText = (status: AvailabilityStatus) => {
  switch (status) {
    case 'Available':
      return (
        <>
          <BiSolidCircle color={'green'} fontSize={'0.7rem'} /> {__('Available')}
        </>
      )
    case 'Away':
      return (
        <>
          <MdWatchLater color={'#FFAA33'} fontSize={'0.8rem'} /> {__('Away')}
        </>
      )
    case 'Do not disturb':
      return (
        <>
          <FaCircleMinus color={'#D22B2B'} fontSize={'0.7rem'} /> {__('Do not disturb')}
        </>
      )
    case 'Invisible':
      return (
        <>
          <FaCircleDot className={'text-gray-400'} fontSize={'0.7rem'} /> {__('Invisible')}
        </>
      )
    default:
      return (
