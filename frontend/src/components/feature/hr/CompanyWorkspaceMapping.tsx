...
const CompanyWorkspaceMapping = () => {
  const {
    control,
    formState: { errors, disabled }
  } = useFormContext<RavenSettings>()
    ...
    <Stack>
      <HStack justify='between' align='center'>
        <Text size='2' weight='medium'>
          {__('Choose workspaces based on companies')}
        </Text>
        <Button className='not-cal' disabled={disabled} type='button' variant='soft' size='2' onClick={addRow}>
          {__('Add')}
        </Button>
      </HStack>
          ...
          <HStack key={field.id} className='px-2 py-2 border-b border-gray-4'>
            ...
            <Box width='45%'>
              <Stack gap='1'>
                ...
                <WorkspaceDropdown name={`company_workspace_mapping.${index}.raven_workspace`} />
