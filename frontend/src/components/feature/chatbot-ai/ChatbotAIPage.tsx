...
import { useChatbotConversations, useCreateChatbotConversation, useChatbotMessages, useSendChatbotMessage, useRenameChatbotConversation } from '@/hooks/useChatbotAPI';
...
const ChatbotAIPage: React.FC = () => {
  ...
  useEffect(() => {
    if (conversations) {
      setDisplaySessions(conversations.map((c: any) => ({
        id: c.name,
        title: c.title,
        messages: []
      })));
  ...
  const sessionsForUI = displaySessions.map(s =>
    pendingRenames[s.id]
      ? { ...s, title: 'Đang xử lý...' }
      : s
  );
  ...
  return (
    <div className="flex h-full w-full">
      ...
      <div className="flex-1 h-full">
        {selectedId ? (
          <ChatbotAIChatBox
            session={{ id: selectedId, title: displaySessions.find(s => s.id === selectedId)?.title || '', messages: displayMessages }}
