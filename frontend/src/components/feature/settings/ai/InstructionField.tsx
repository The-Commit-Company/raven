    ...
    <Stack gap='4'>
      <Flex direction={'column'} gap='2'>
        ...
        <HelperText size='2'>
          ...
          <br />
          <br />
          Instructions would be different based on the user who is calling the bot or the data in your system as they
          are computed every time the bot is called.
        </HelperText>
      </Flex>

      {isDynamic ? (
        <DynamicInstructionField
          autoFocus={autoFocus}
          instructionRequired={instructionRequired}
          allowUsingTemplate={allowUsingTemplate}
        />
      ) : (
        <StaticInstructionField
          autoFocus={autoFocus}
          instructionRequired={instructionRequired}
          allowUsingTemplate={allowUsingTemplate}
        />
  ...
  return (
    <Stack gap='4'>
      ...
      <Text size='2'>
        ...
        <br />
        You can also use standard Jinja variables available in the system.
      </Text>
...
export const VariableRow = ({
  ...
  withoutJinja?: boolean
}) => {
  return (
    <Table.Row>
      <Table.Cell>
        <VariableTooltip text={variable} withoutJinja={withoutJinja} />
      </Table.Cell>
...
export const VariableTooltip = ({ text, withoutJinja = false }: { text: string; withoutJinja?: boolean }) => {
  ...
  const copyText = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    window.navigator.clipboard
      .writeText(withoutJinja ? text : '{{ ' + text + ' }}')
      .then(() => {
        setTooltip('Copied!')
        setTimeout(() => {
          setTooltip('')
  ...
  return (
    ...
    >
      <Code role='button' onClick={copyText} aria-label='Copy to clipboard' className='cursor-pointer'>
        {text}
      </Code>
    </Tooltip>
  )
...
const InstructionPreview = () => {
  ...
  return (
    <Box className='border-2 px-2 py-1 rounded-md bg-gray-3'>
      {data ? (
...
const StaticInstructionField = ({
  allowUsingTemplate,
  instructionRequired,
  ...props
}: TextAreaProps & { allowUsingTemplate?: boolean; instructionRequired?: boolean }) => {
  const {
    register,
    watch,
    formState: { errors }
  } = useFormContext<InstructionFieldForm>()

  const isDynamic = watch('dynamic_instructions')

  const placeholder = isDynamic
    ? "You are an assistant running on an ERP. The current user's name is {{ first_name }} and the current company is {{ company }}."
    : 'You are an assistant running on an ERP. You can answer questions about the company.'

  return (
    <Stack>
      <Box>
        <HStack justify={'between'}>
          <Label htmlFor='instruction' isRequired>
            Instruction
          </Label>
...
const ImportTemplate = () => {
  ...
  return (
    <Popover.Root onOpenChange={() => setSelectedTemplate('')}>
      <Popover.Trigger>
        <Button variant='ghost' size={'1'} className='not-cal font-medium'>
      ...
      <Popover.Content width={'300px'} className='z-50'>
        <Stack gap='6'>
          ...
          <Box>
            ...
            >
              {data?.map((t) => (
                <RadioCards.Item key={t.name} value={t.instruction} className='px-3 py-3'>
                  <HStack width='100%'>
