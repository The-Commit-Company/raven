...
const VariableDialog = ({ defaultValues, onAdd, allowNameChange, name: defaultName, defaultRequired }: Props) => {
  const [type, setType] = useState<'object' | 'array' | 'string' | 'number' | 'boolean'>(
  ...
  const [enumValues, setEnumValues] = useState<string[]>(
    defaultValues?.type === 'string' || defaultValues?.type === 'number' ? (defaultValues?.enum ?? []) : []
  )

  const [items, setItems] = useState<StringVariableType | NumberVariableType | undefined>(
    defaultValues?.type === 'array' ? (defaultValues?.items ?? []) : undefined
  )
  ...
  return (
    <Stack gap='4' pt='2'>
      <Stack gap='4' className='max-h-[75vh] overflow-y-auto'>
        <HStack>
          <Stack width='50%' gap='4'>
            <Box>
              ...
              <Select.Root
                required
                value={type}
                onValueChange={(value) => setType(value as 'object' | 'array' | 'string' | 'number' | 'boolean')}
              >
            ...
            <Box>
              <Text as='label' size='2'>
                <HStack>
                  <Checkbox checked={required} onCheckedChange={(v) => setRequired(v ? true : false)} />
                  Required
                </HStack>
              </Text>
            </Box>
          ...
          <Stack width='50%'>
            ...
            <HelperText>Variable name must be unique, and cannot have spaces.</HelperText>
          </Stack>
        </HStack>
        ...
        {(type === 'string' || type === 'number') && (
          <Stack>
            <Stack>
              ...
              <HelperText>
                Add a new option on a new line. If you want this variable to have a default value, just add one option.
              </HelperText>
            </Stack>
          </Stack>
        )}
      ...
      <HStack justify='end'>
        ...
        <Button onClick={onSubmit} disabled={areAllFieldsFilled}>
          Add
        </Button>
      </HStack>
    </Stack>
