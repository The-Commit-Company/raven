        ...
        <HStack justify='between' align='start'>
          <HStack align='center'>
            ...
            <Switch
              ...
              onCheckedChange={(checked) => setViewMode(checked ? 'json' : 'builder')}
            />
            <Text weight='medium' size='2'>
              JSON
            </Text>
          ...
          {viewMode === 'json' && (
            <Button color='gray' className='not-cal font-medium' variant='soft' type='button' onClick={formatJSON}>
              Format JSON
            </Button>
          )}
        </HStack>
              ...
              try {
                ...
                return (
                  <ErrorBoundary
                    fallback={
                      <Text size='2' color='red'>
                        Error loading variables in the builder. Please check your JSON structure.
                      </Text>
                    }
                  >
...
const VariableBuilderField = ({
  json,
  onChange,
  isNested
}: {
  json: ObjectVariableType
  onChange: (json: ObjectVariableType) => void
  isNested?: boolean
}) => {
  const [editingVariable, setEditingVariable] = useState<{ name: string; properties: VariableType } | undefined>(
    undefined
  )
      ...
      <Stack gap='1'>
        ...
        {Object.entries(properties)?.map(([key, value]) => {
          return (
            <Stack
              key={key}
              className='bg-gray-2 dark:bg-gray-4 p-2.5 rounded-md shadow-sm hover:bg-gray-3 dark:hover:bg-gray-5'
            >
              <HStack justify='between' align='center'>
                ...
                <HStack align='center' pr='2' gap='4'>
                  ...
                  <IconButton size='2' color='red' variant='ghost' type='button' onClick={() => removeVariable(key)}>
                    <BiTrashAlt size='16' />
                  </IconButton>
                </HStack>
              </HStack>
              {value.type === 'object' && (
                <Stack>
                  ...
                  <Box ml='8' p='2' className='bg-white dark:bg-gray-2 rounded-md'>
                    <VariableBuilderField isNested json={value} onChange={(v) => addVariable(key, v)} />
                  </Box>
                </Stack>
              )}
            </Stack>
...
const AddVariablePopover = ({ onAdd, buttonProps }: AddVariablePopoverProps) => {
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      <Dialog.Trigger>
        <Button size='2' variant='soft' {...buttonProps}>
          Add Variable
        </Button>
      </Dialog.Trigger>
