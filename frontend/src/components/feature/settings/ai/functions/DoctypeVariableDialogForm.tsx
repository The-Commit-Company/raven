...
const TableSelectionField = ({ doctype }: { doctype: string }) => {
  ...
  return (
    <Box width='50%'>
      <Label isRequired htmlFor='child_table_name'>
        DocType
      </Label>
      <Controller
        control={control}
...
const FieldSelectionField = ({ doctype }: { doctype: string }) => {
  ...
  const doctypeName = useMemo(() => {
    ...
    const childDocTypeName = doctypeMeta?.fields?.find((field) => field.fieldname === tableField)?.options
  ...
  return <DoctypeFieldSelect doctype={doctypeName} value={value} onFieldSelect={onFieldSelect} />
}

/**
 * Component to render a select field for the fields of a doctype. Could be the main doctype or a child table.
 */
export const DoctypeFieldSelect = ({
  doctype,
  value,
  onFieldSelect
}: {
  doctype: string
  value: string
  onFieldSelect: (field: DocField) => void
}) => {
  const { doc: doctypeMeta } = useDoctypeMeta(doctype)
  ...
  return (
    <Box width='50%'>
      <Label isRequired htmlFor='fieldname'>
        Field
      </Label>
...
const OtherFormFields = () => {
  const {
    ...
    formState: { errors },
    watch
  } = useFormContext<RavenAIFunctionParams>()
    ...
    <Stack gap='4'>
      <HStack>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select.Root
                      ...
                      value={field.value}
                      onValueChange={(value) =>
                        field.onChange(value as 'object' | 'array' | 'string' | 'number' | 'boolean')
                      }
                    >
        ...
        <Box width='50%'>
          <Stack gap='1'>
            <Box>
              <Label htmlFor='default_value' isRequired={do_not_ask_ai ? true : false}>
                Default Value
              </Label>
              <TextField.Root
            ...
            <HelperText>
              You can specify a default value for this variable. If the AI does not fill this field, then the default
              value will be used.
            </HelperText>
          </Stack>
        </Box>
      ...
      <Stack gap='1'>
        ...
        <HelperText>
          This is used to describe what this variable is used for. A better description will help the AI Bot perform
          better.
        </HelperText>
      </Stack>
    </Stack>
...
const OptionsField = ({ doctype }: { doctype: string }) => {
  ...
  const doctypeName = useMemo(() => {
    ...
    const childDocTypeName = doctypeMeta?.fields?.find((field) => field.fieldname === child_table_name)?.options
  ...
  return (
    <Stack gap='1' width='100%'>
      <Box width='100%'>
        ...
        <TextArea {...register('options')} rows={5} placeholder={`Add options separated by a new line.`} />
      </Box>
      {errors.options && <ErrorText>{errors.options?.message}</ErrorText>}
      <HelperText>
        You can limit the values that the bot can fill in this field by adding options.
        <br />
        This helps the bot to make less mistakes.
      </HelperText>
      <Box pt='2'>{fieldname && <OptionsAutoFill doctype={doctypeName} fieldname={fieldname} />}</Box>
    </Stack>
  )
}
...
const OptionsAutoFill = ({ doctype, fieldname }: { doctype: string; fieldname: string }) => {
  ...
  if (data?.message === 21) {
    return (
      <Callout.Root className='block' size='1'>
        <HStack align='center' width='100%' justify='between'>
          <Callout.Text weight={'medium'}>Quickly import {linkDoctypeName}s?</Callout.Text>
          <Box>
    ...
    <Callout.Root className='block' size='1'>
      <HStack align='center' width='100%' justify='between'>
        <Callout.Text>
          ...
          <br />
          Do you want to autofill these as options?
        </Callout.Text>
        <Box>
          <Button type='button' variant='surface' className='not-cal' onClick={autoFillOptions}>
            Auto Fill Options
          </Button>
        </Box>
      </HStack>
...
const QuickImportPopover = ({ doctype }: { doctype: string }) => {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger>
        <Button type='button' variant='surface' className='not-cal'>
          Select {doctype}s
        </Button>
      </Popover.Trigger>
