  ...
  return (
    <Stack>
      <HStack justify='between' align='center'>
        ...
        <HStack gap='2'>
          <FileSourceUploadDialog buttonProps={{ variant: 'soft', className: 'not-cal' }} onUpload={addNew} />

          <Dialog.Root>
            <Dialog.Trigger>
              <Button className='not-cal' type='button'>
                Select Files
              </Button>
            </Dialog.Trigger>
            <Dialog.Content>
      ...
      <Table.Root variant='surface' className='rounded-sm animate-fadein'>
        ...
        <Table.Body>
          {fields?.map((d, index) => <FileSourceRow key={d.name} fileID={d.file} onDelete={() => remove(index)} />)}
        </Table.Body>
      </Table.Root>
    </Stack>
  )
...
const FileSourceRow = ({ fileID, onDelete }: { fileID: string; onDelete: () => void }) => {
  ...
  return (
    <Table.Row className='hover:bg-gray-2 dark:hover:bg-gray-3'>
      ...
      <Table.Cell maxWidth={'250px'}>
        <Badge color='gray' className='uppercase'>
          {file?.file_type}
        </Badge>
      </Table.Cell>
  ...
  return (
    <Stack>
                  ...
                  <Label htmlFor={d.name}>
                    <HStack align='center'>
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => onSelect(d.name)}
                      />
                      <Text as='span' size='2' className='text-ellipsis overflow-hidden whitespace-nowrap'>
                        {d.file_name}
                      </Text>
                    </HStack>
                  </Label>
      ...
      <HStack justify='end' gap='2' pt='4'>
        <Dialog.Close>
          <Button variant='soft' color='gray' type='button' className='not-cal'>
            Close
          </Button>
        </Dialog.Close>
