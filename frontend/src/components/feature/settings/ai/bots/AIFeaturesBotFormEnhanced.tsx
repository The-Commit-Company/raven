...
const AIFeaturesBotFormEnhanced = (props: Props) => {
                ...
                <Controller control={control} name='model_provider'
                    ...
                    render={({ field }) => (
                        <Select.Root
                            ...
                            <Select.Content>
                                {hasOpenAI && <Select.Item value='OpenAI'>OpenAI</Select.Item>}
                                {hasLocalLLM && <Select.Item value='Local LLM'>Local LLM</Select.Item>}
                            </Select.Content>
                        </Select.Root>
                    )} />
            </Box>
...
const ModelSelector = () => {
    ...
    const { data: openaiModels } = useFrappeGetCall<{ message: string[] }>(
        ...
        undefined,
        {
            isReady: modelProvider === 'OpenAI' || !modelProvider
        }
    )

    // Fetch Local LLM models
    const { call: fetchLocalModels } = useFrappePostCall<{
        message: {
            success: boolean
    ...
    useEffect(() => {
        if (modelProvider === 'Local LLM' && ravenSettings?.local_llm_api_url) {
            fetchLocalModels({
                provider: 'Local LLM',
                api_url: ravenSettings.local_llm_api_url
            }).then(result => {
                if (result.message.success && result.message.models) {
                    setLocalModels(result.message.models.map(m => m.id))
                }
            }).catch(() => {
                setLocalModels([])
            })
        }
    }, [modelProvider, ravenSettings?.local_llm_api_url])

    if (!is_ai_bot) return null

    const models = modelProvider === 'Local LLM' ? localModels : openaiModels?.message || []
...
export default AIFeaturesBotFormEnhanced