...
const subTitles: Record<
  TicketType,
  { heading: string; subHeading: string; defaultTextAreaValue: string; footerHeading: ReactNode }
> = {
  Feedback: {
    heading: 'Send feedback',
    subHeading:
      "How can we improve Linear? If you have a feature request, can you also share how you would use it and why it's important to you?",
    defaultTextAreaValue: 'What if...',
    footerHeading: (
      <span>
        You can also email us at{' '}
        <Link href='mailto:support@thecommit.company' underline='none' size='1' target='_blank'>
          support@thecommit.company
        </Link>{' '}
        We can't respond to every request but we read all of them.
      </span>
    )
  },
  Question: {
...
const SupportRequestForm = ({ onClose }: SupportRequestFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <Dialog.Description size={'2'} className='min-h-10 select-none'>
        {subTitles[requestType].subHeading}
      </Dialog.Description>
      <Stack gap='2' pt='3'>
        <Controller
          name='ticket_type'
          ...
          render={({ field: { onChange, value } }) => (
            <RadioCards.Root value={value} onValueChange={onChange}>
        ...
        <HStack justify='between' pt='4' gap='9'>
          <Text color='gray' size='1' className='select-none'>
            {subTitles[requestType].footerHeading}
          </Text>
          <Flex gap='2'>
            <Dialog.Close>
