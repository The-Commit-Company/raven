...
const DocumentPreviewToolConfigurator = () => {
  ...
  return (
    <Grid columns='2' gap='4'>
      <Stack>
        <Heading size='3' className='not-cal'>
          Configure
        </Heading>
        <LinkField
...
const DocTypePreviewFields = ({ doctype, docname }: { doctype: string; docname: string }) => {
  ...
  const { previewFields, eligibleFields } = useMemo(() => {
    const eligibleFields =
      doc?.fields?.filter((field) => !NO_VALUE_FIELDS.includes(field.fieldtype) && field.fieldname) ?? []
...
const DocTypePreviewEditor = ({
  ...
  const handleUpdate = () => {
    call({ doctype, fields: selectedFields })
      .then(() => {
        ...
        mutateMeta()
        mutate(
          `raven.api.document_link.get_preview_data?doctype=${encodeURIComponent(doctype)}&docname=${encodeURIComponent(docname)}`
        )
      })
      .catch((error) => {
  ...
  return (
    <Stack>
      ...
      <FieldSelector
        eligibleFields={filteredFields}
        selectedFields={selectedFields}
        setSelectedFields={onFieldChange}
      />
    </Stack>
  )
}
...
const PreviewFieldRow = ({ field }: { field: DocField }) => {
  return (
    <CheckboxCards.Item value={field.fieldname ?? ''}>
...
const DocTypePreview = ({
  ...
  return (
    <Stack>
      ...
      <LinkField label='Select a Document' doctype={doctype} value={docname} setValue={setDocname} />
