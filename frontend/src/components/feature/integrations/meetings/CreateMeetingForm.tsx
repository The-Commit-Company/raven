...
const CreateMeetingForm = ({ onClose, channelData }: CreateMeetingFormProps) => {
  ...
  useFrappeGetCall(
    ...
    {
      onSuccess: (data) => {
        if (data.message) setValue('google_calendar', data.message.name)
      }
    }
  )
          ...
          <Box width='100%'>
            <Flex direction='column' gap='2'>
              <Box>
                ...
                <TextArea {...register('subject', { required: 'Subject is required' })} />
              </Box>
              ...
              <Flex direction='column' gap='2'>
                <Box width='100%'>
                  ...
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => (
                      <Select.Root value={field.value} name={field.name} required onValueChange={field.onChange}>
