...
export const ViewWebhookPage = ({
  data,
  mutate
}: {
  data: FrappeDoc<RavenWebhook>
  mutate: KeyedMutator<FrappeDoc<RavenWebhook>>
}) => {
  ...
  const onSubmit = async (data: FieldValues) => {
    return updateDoc('Raven Webhook', data.name, data).then((doc) => {
  ...
  const Badges: HeaderBadge[] = data?.enabled
            ...
            <AlertDialog.Content className={DIALOG_CONTENT_CLASS}>
              ...
              <Flex gap='3' mt='4' justify='end'>
                ...
                <AlertDialog.Action>
                  <Button
                    variant='solid'
                    color={data.enabled ? 'red' : undefined}
