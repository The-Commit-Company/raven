...
export const WebhookForm = ({ isEdit = false }: { isEdit?: boolean }) => {
  const {
    ...
    setValue,
    watch
  } = useFormContext<RavenWebhook>()

  return (
    <Tabs.Root defaultValue='general'>
      <Tabs.List>
        <Tabs.Trigger value='general'>
          <AiOutlineApi {...ICON_PROPS} /> General
        </Tabs.Trigger>
        <Tabs.Trigger value='condition'>
          <LuWorkflow {...ICON_PROPS} /> Conditions
        </Tabs.Trigger>
        <Tabs.Trigger value='data'>
          <AiOutlineDatabase {...ICON_PROPS} /> Data
        </Tabs.Trigger>
        <Tabs.Trigger value='headers'>
          <BiCodeCurly {...ICON_PROPS} /> Headers
        </Tabs.Trigger>
      </Tabs.List>
      <Box pt='4'>
...
const GeneralWebhookForm = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  return (
    <Stack gap='4'>
      ...
      <Stack>
        <Box>
          <Label htmlFor='request_url' isRequired>
            Request URL
          </Label>
          <TextField.Root
      ...
      <Stack maxWidth={'360px'}>
        <Box>
          <Flex direction={'column'}>
            <Label isRequired htmlFor='webhook_trigger'>
              Trigger Event
            </Label>
            <Controller
              control={control}
                  ...
                  <Select.Content>
                    <Select.Group>
                      ...
                      {TriggerEvents?.map((event, index) => (
                        <Select.Item key={index} value={event.label}>
                          {event.label}
                        </Select.Item>
                      ))}
                    </Select.Group>
      ...
      <Stack pt='2'>
        ...
        <HelperText size='2'>
          To optionally add security to your webhook requests and ensure that the webhook is being sent from Raven, you
          can set up a "Webhook Secret" along with the request. Do not share the secret publicly.
          <br />
          <br />
          If enabled, an additional header (X-Frappe-Webhook-Signature) will be added to the request before it's sent
          out, with its value being generated from the secret as a base64-encoded HMAC-SHA256 hash of the payload.
        </HelperText>
        {errors?.enable_security && <ErrorText>{errors.enable_security.message}</ErrorText>}
      </Stack>
      {security ? (
        <Box>
          <Label htmlFor='webhook_secret' isRequired>
            Webhook Secret
          </Label>
          <TextField.Root
            type='password'
                ...
                <Select.Content>
                  <Select.Group>
                    ...
                    {triggerOn?.map((event, index) => (
                      <Select.Item key={index} value={event}>
                        {event}
                      </Select.Item>
                    ))}
      ...
      {conditionOn === 'Custom' ? (
        <Box>
          <Flex direction={'row'} gap={'4'} align={'start'}>
            ...
            >
              <Label htmlFor='condition' isRequired>
                Condition
              </Label>
              <TextArea
                id='condition'
        ...
        <Flex direction={'column'}>
          ...
          >
            Webhook will trigger only if the message is sent by this user.
          </HelperText>
        </Flex>
    ...
    <HStack justify='between' width='100%'>
      <Flex direction={'row'} gap={'1'} align={'center'}>
        ...
        <Flex justify='between' align={'center'} width='100%'>
          <Text size='2' className='text-ellipsis line-clamp-1' as='span'>
      ...
      <HStack gap='1' align='center'>
        ...
        <Text size='1' className='opacity-75'>
          {channel.workspace}
        </Text>
      </HStack>
    </HStack>
