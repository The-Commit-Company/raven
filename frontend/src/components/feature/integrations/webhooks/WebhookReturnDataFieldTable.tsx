      ...
      <Flex direction='column' gap='2' width='100%'>
        <HStack align='center' justify={'between'}>
          <Flex direction={'column'} gap='1'>
            <Heading size='4' className='not-cal'>
              Webhook Data
            </Heading>
            <Text size='2' color='gray'>
              Select the fields you want in the webhook payload.
            </Text>
          </Flex>
          <HStack align={'center'}>
                          ...
                          <Select.Content>
                            <Select.Group>
                              ...
                              {webhookDataFieldName?.map((field, index) => (
                                ...
                                >{`${field.label} (${field.fieldtype})`}</Select.Item>
                              ))}
                            </Select.Group>
                          </Select.Content>
                        </Select.Root>
...
export const FieldInfoModal = ({
  ...
  const fieldData = useMemo(() => {
    ...
    return DoctypeFieldList?.find((field) => field.events.includes(triggerEvent))?.fields.find(
  ...
  return (
    <Flex direction={'column'} gap={'4'} width={'100%'}>
      <Box>
        <Dialog.Title>{fieldData?.label}</Dialog.Title>
        <Dialog.Description>
      ...
      {fieldData?.example && (
        <Box>
          ...
          <Box>
            <pre
              className={'rounded-md bg-slate-3 p-2 m-0'}
              style={{
                minHeight: '200px'
              }}
            >
...
export const PreviewModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const exampleList = useMemo(() => {
    return (
      SampleData?.find((sample) => sample.trigger_event.includes(webhookTrigger))?.examples?.map(
  ...
  const jsonData = useMemo(() => {
    const exampleData = SampleData?.find((sample) => sample.trigger_event.includes(webhookTrigger))?.examples?.find(
      (example) => example.name === examples
    )
  ...
  return (
    <Stack>
      <Box>
        ...
        <Dialog.Description>Preview the webhook payload for the selected example.</Dialog.Description>
      </Box>
      <Flex direction={'column'} gap={'4'} width={'100%'}>
        <Flex direction={'column'} gap={'2'} width={'100%'}>
          <Box>
