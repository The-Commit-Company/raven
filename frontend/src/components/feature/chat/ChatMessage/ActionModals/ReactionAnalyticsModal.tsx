...
const TabTrigger = ({
  ...
  <Tabs.Trigger value={emojiSrc} className='text-gray-11' title={emojiName}>
    <Flex gap='2' align='center' justify='center'>
      {isCustom ? (
        <img src={emojiSrc} alt={emojiName} className='w-[1.2rem] h-[1.2rem] object-contain object-center' />
      ) : emojiName === 'All' ? (
        <Text size='3'>All</Text>
      ) : (
        <Text size='3' className='w-[1.2rem] h-[1.2rem]'>
          {/* @ts-expect-error */}
          <em-emoji native={emojiName} set='apple'></em-emoji>
        </Text>
      )}
      ...
      <Flex align='center' justify='between'>
        ...
        {is_custom ? (
          <img
            ...
            alt={emoji_name}
            title={emoji_name}
            className='mr-3 w-[1.4rem] h-[1.4rem] object-contain object-center'
          />
        ) : (
