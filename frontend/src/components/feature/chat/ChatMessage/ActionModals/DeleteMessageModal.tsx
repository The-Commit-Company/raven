...
export const DeleteMessageModal = ({ onClose, message }: DeleteMessageModalProps) => {
  ...
  return (
    <>
      <AlertDialog.Title>Delete {message.is_thread ? 'Thread' : 'Message'}</AlertDialog.Title>
      ...
      <Flex direction={'column'} gap='2'>
        <Callout.Root color='red' size='1'>
          ...
          <Callout.Text size='2'>This action is permanent and cannot be undone.</Callout.Text>
        </Callout.Root>

        <ErrorBanner error={error} />
        {message.is_thread ? (
          <Text size='2'>This message is a thread, deleting it will delete all messages in the thread.</Text>
        ) : (
