  ...
  if (isDesktop) {
    return (
      <Dialog.Root open={isOpen} onOpenChange={onClose}>
        <Dialog.Content className={'static'}>
          {message && <AttachFileToDocumentModal message={message} onClose={onClose} />}
        </Dialog.Content>
      </Dialog.Root>
    )
  } else {
    return (
      <Drawer open={isOpen} onClose={onClose}>
        <DrawerContent>
          <div className='pb-24'>{message && <AttachFileToDocumentModal message={message} onClose={onClose} />}</div>
        </DrawerContent>
      </Drawer>
    )
  }
