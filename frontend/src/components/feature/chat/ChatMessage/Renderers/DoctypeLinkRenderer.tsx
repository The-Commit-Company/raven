          ...
          <Flex gap='2' align='center' width={'100%'}>
            ...
            <Grid gap='1' width={'100%'}>
              <Flex gap='1'>
                ...
                <Text size='2' color='gray' className='cursor-copy' onClick={() => onCopyTextClick('ID')}>
                  {data?.id}
                </Text>
              </Flex>
              <Heading as='h3' size='3' className='leading-4 pl-0.5 my-0'>
          ...
          {data &&
            Object.keys(removePreviewFields(data))?.map((item, index) => (
              <DataList.Item align='center' key={item}>
                ...
                <DataList.Value>{typeof data[item] === 'string' ? parse(data[item]) : data[item]}</DataList.Value>
              </DataList.Item>
            ))}
        </DataList.Root>
      </Grid>
...
const DoctypeActionMenu = ({
  ...
  doctype: string
  docname: string
  onCopyLinkClick: VoidFunction
  mutate: VoidFunction
}) => {
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger>
        <IconButton size='1' title='More actions' aria-label='More actions' color='gray' variant='ghost'>
    ...
    <DropdownMenu.Sub open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenu.SubContent>
        <WorkflowTransitionOptions
          doctype={doctype}
          docname={docname}
          workflow={doc?.__workflow_docs?.[0]}
          mutate={mutate}
        />
      </DropdownMenu.SubContent>
    </DropdownMenu.Sub>
...
const WorkflowTransitionOptions = ({
  doctype,
  docname,
  mutate
}: {
  ...
  const { data: transitions, mutate: refreshTransitions } = useFrappeGetCall<{ message: WorkFlowButtonData[] }>(
      ...
      {transitions?.message?.map((transition) => {
        return (
          <DropdownMenu.Item key={transition.name} onClick={() => applyTransition(transition.action)}>
            ...
            <HStack gap='1'>
              <Text size='1' className='font-light'>
                {transition.state}
              </Text>
