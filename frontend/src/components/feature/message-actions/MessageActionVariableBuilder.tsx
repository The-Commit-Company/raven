...
const MessageActionVariableBuilder = () => {
  ...
  return (
    <Stack>
      <HStack justify='end' align='center'>
        {action === 'Create Document' && doctype && <ImportDoctypeVariables append={addField} />}
              ...
              return (
                <Table.Row key={field.id}>
                  <Table.Cell>
                    ...
                    <Badge color={getColorForFieldType(field.type)} className='ml-2 rounded-md'>
                      {field.type}
                    </Badge>
                  </Table.Cell>
                  <Table.Cell>
                    <Code variant='ghost'>{field.fieldname}</Code>
                  </Table.Cell>
                  <Table.Cell>
                    <Checkbox checked={!!field.is_required} />
                  </Table.Cell>
                  <Table.Cell>
                    {field.options ? (
                      field.type === 'Select' ? (
                        ...
                        <Badge color='iris' variant='soft' className='rounded-md'>
                          {field.options}
                        </Badge>
                      )
                    ) : null}
...
const ImportDoctypeVariables = ({ append }: { append: (data: Partial<RavenMessageActionFields>) => void }) => {
  ...
  const importFields = () => {
    ...
    const requiredFields = doctypeMeta?.fields?.filter(
      (f) => f.reqd && f.fieldtype !== 'Table' && f.fieldtype !== 'Table MultiSelect'
    )
...
const AddFieldDialog = ({
  ...
  onAdd: (data: Partial<RavenMessageActionFields>) => void
  doctype?: string
}) => {
...
const EditFieldDialog = ({
  ...
  onEdit: (data: Partial<RavenMessageActionFields>) => void
  field: RavenMessageActionFields
  doctype?: string
}) => {
  const [open, setOpen] = useState(false)
    ...
    <FormProvider {...methods}>
      <Stack gap='4'>
        <Stack gap='4'>
          <HStack>
            {doctype ? (
              <DoctypeFieldSelect doctype={doctype} value={fieldname} onFieldSelect={onFieldSelect} />
            ) : (
              <Stack>
                <Box>
              ...
              <Stack width={'50%'}>
                <Box>
                  <Label htmlFor='options'>
                    Validation{' '}
                    <Text as='span' weight='regular' size='2' color='gray'>
                      {' '}
                      (optional)
                    </Text>
                  </Label>
          ...
          <Stack>
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <Select.Root value={value} onValueChange={onChange}>
                    <Select.Trigger
                      placeholder='Select a default value type'
                      className='w-full'
            ...
            <HelperText>
              You can choose between a static default value, message field or jinja template.
              <br />
              Message field will allow you to set a default value from the message selected.
              <br />
              Jinja tags can be used with the message as context.
            </HelperText>
          ...
          <Stack>
            <Box>
              ...
              {default_value_type === 'Static' && <TextField.Root {...register('default_value')} id='default_value' />}
