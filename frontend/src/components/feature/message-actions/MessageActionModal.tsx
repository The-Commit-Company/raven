...
const MessageActionModal = ({ messageID, actionID, onClose }: MessageActionModalProps) => {
  const { data: action } = useFrappeGetDoc<RavenMessageAction>(
  ...
  return (
    <Dialog.Root open={actionID !== ''} onOpenChange={onClose}>
      {action && (
...
const FieldRenderer = ({ field, autoFocus }: { field: RavenMessageActionFields; autoFocus?: boolean }) => {
  ...
  const ariaDescribedby = hasError
    ...
    : field.helper_text
      ? `${field.fieldname}-helper`
      : undefined

  const FieldLabel = () => {
    return (
      <Label isRequired={field.is_required === 1} htmlFor={field.fieldname}>
        {field.label}
      </Label>
    )
  }
        ...
        <Box>
          ...
          <Controller
            ...
            render={({ field: { value, onChange, onBlur, name, disabled, ref } }) => (
              <Select.Root value={value} name={name} required={field.is_required === 1} onValueChange={onChange}>
                <Select.Trigger
                  onBlur={onBlur}
                  className='w-full'
                  id={field.fieldname}
