...
export const FileSearch = ({
  ...
  const [channelFilter, setChannelFilter] = useState<string | undefined>(
    inFilter ? inFilter : dm_channels.find((c) => c.peer_user_id == withFilter)?.name
  )
  ...
  const { data, error, isLoading } = useFrappeGetCall<{ message: GetFileSearchResult[] }>(
    'raven.api.search.get_search_result',
    {
      filter_type: 'File',
  ...
  return (
    <Box>
      <Flex direction='column' gap='2'>
        ...
        <Grid gap='2' justify='between' columns={{ initial: '2', md: '5' }} align='center'>
                  ...
                  <Select.Item key={option.name} value={option.name}>
                    <Flex gap='2' align='center' className='overflow-hidden'>
                      ...
                      <Text
                        ...
                        className='text-ellipsis whitespace-break-spaces line-clamp-1'
                      >
                        {option.channel_name}
                      </Text>
                    </Flex>
              ...
              <Select.Group>
                ...
                {dm_channels?.map((option) => (
                  ...
                  >
                    <Flex gap='2' align='center'>
                      <UserAvatar
                        src={users[option.peer_user_id]?.user_image}
                        alt={users[option.peer_user_id]?.full_name ?? option.peer_user_id}
                      />
              ...
              return (
                <Flex gap='3' key={f.name} align='center'>
                  ...
                  <Flex direction='column' className='w-[94%]'>
                    {f.file && (
                      <Link weight='medium' size='1' href={f.file} target='_blank'>
                        {getFileName(f.file)}
                      </Link>
                    )}
                    {users && (
                      <Text size='1' color='gray'>
                        Shared by {Object.values(users).find((user: UserFields) => user.name === f.owner)?.full_name} on{' '}
                        <DateMonthYear date={f.creation} />
                      </Text>
                    )}
                  </Flex>
                </Flex>
