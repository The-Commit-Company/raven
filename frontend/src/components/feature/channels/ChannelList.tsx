...
const PinButton = ({ channelID }: { channelID: string }) => {
  ...
  if (isPinned) {
    return (
      <ContextMenu.Item onClick={onClick} className='flex justify-start gap-2 min-w-24'>
        <RiUnpinLine size='18' />
        {__('Remove Pin')}
      </ContextMenu.Item>
    )
  ...
  return (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content>
        {showOnlyMyChannels ? (
          <DropdownMenu.Item onClick={showAllChannels}>Show All Channels</DropdownMenu.Item>
        ) : (
