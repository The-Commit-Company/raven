...
def set_user_active():
	...
	frappe.cache().set_value(
		f"raven_user_session_{frappe.session.user}", frappe.session.user, expires_in_sec=900
	)


def set_user_inactive():
	# Remove the user's session ID from the cache
	frappe.cache().delete_key(f"raven_user_session_{frappe.session.user}")


@frappe.whitelist()
def get_active_users():
	# Get all the cache keys that match the pattern 'raven_user_session_*'
	user_session_keys = frappe.cache().get_keys("raven_user_session_*")
