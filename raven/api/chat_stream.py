...
def get_messages(channel_id: str, limit: int = 20, base_message: str | None = None):
    ...
    ).orderby(message.creation, order=Order.desc).orderby(message.name, order=Order.desc).limit(limit).run(as_dict=True)
...
def get_messages_around_base(channel_id: str, base_message: str):
    ...
    newer_messages = fetch_newer_messages(channel_id, base_message, from_timestamp, 10, include_from_message=True)
...
def fetch_older_messages(channel_id: str, from_message: str, from_timestamp: datetime.datetime, limit: int = 20):
    # Cannot use `get_all` as it does not apply the `order_by` clause to multiple fields
    message = frappe.qb.DocType('Raven Message')
    ...
    messages = frappe.qb.from_(message).select(
        ...
        message.channel_id == channel_id
    ).where((message.creation < from_timestamp) | ((message.creation == from_timestamp) & (message.name < from_message))).orderby(message.creation, order=Order.desc).orderby(message.name, order=Order.desc).limit(limit).run(as_dict=True)
    ...
    if len(messages) == limit:
        ...
        older_message = frappe.db.get_all('Raven Message',
                                          pluck='name',
                                            filters={'channel_id': channel_id, 'creation': ('<=', messages[-1].creation), 'name': ['!=', messages[-1].name]},
                                            order_by='creation desc, name desc',
                                            limit=1
                                            )
...
def get_newer_messages(channel_id: str, from_message: str, limit: int = 20):
    ...
    return fetch_newer_messages(channel_id, from_message, from_timestamp, limit, include_from_message=False)

def fetch_newer_messages(channel_id: str, from_message: str, from_timestamp: datetime.datetime, limit: int = 20, include_from_message: bool = False):

    message = frappe.qb.DocType('Raven Message')

    if include_from_message:
        condition = ((message.creation > from_timestamp) | ((message.creation == from_timestamp) & (message.name >= from_message)))
    ...
    messages = frappe.qb.from_(message).select(
        ...
        message.channel_id == channel_id
    ).where(condition).orderby(message.creation, order=Order.asc).orderby(message.name, order=Order.asc).limit(limit).run(as_dict=True)
