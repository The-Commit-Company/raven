...
def after_insert(doc, method):
    ...
    department_channel.channel_description = "Channel for Department - {}".format(doc.department_name)
...
def on_update(doc, method):
    linked_channels = frappe.get_all("Raven Channel", {
    ...
    for channel in linked_channels:
        ...
        frappe.db.set_value("Raven Channel", channel, {
            "channel_name": channel_name,
        })

def on_trash(doc, method):
    frappe.db.delete("Raven Channel", {
        "linked_doctype": "Department",
        "linked_document": doc.name
    })
...
def get_auto_create_department_channel_type() -> str | None:
    ...
    if raven_settings.auto_create_department_channel:
        return raven_settings.department_channel_type if raven_settings.department_channel_type else "Private"
    else:
        return None
