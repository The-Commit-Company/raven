...
def send_push_notification_via_raven_cloud(message, raven_settings):
    ...
    try:
        ...
        frappe.log_error(
            title="DM Debug - Messages End",
            message=f"messages: {json.dumps(messages)}",
        )
...
def subscribe_user_to_topic(channel_id, user_id):
    ...
    """
    notification_service = frappe.db.get_single_value(
        "Raven Settings", "push_notification_service"
    )
