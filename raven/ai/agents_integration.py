...
from agents import Agent, Tool, function_tool, RunContextWrapper, set_default_openai_client, ModelSettings, Runner, CodeInterpreterTool

from openai import AsyncOpenAI
from .functions import (
    get_document, get_documents, create_document, create_documents,
    update_document, update_documents, delete_document, delete_documents,
    submit_document, cancel_document, get_amended_document, 
    attach_file_to_document, get_list
)
...
class RavenAgentManager:
    ...
    def _setup_tools(self):
        ...
        if (hasattr(self.bot_doc, 'enable_file_search') and self.bot_doc.enable_file_search 
    ...
    def _create_crud_tools(self) -> List[Tool]:
        ...
        # Tool for get_list
        @function_tool
        def search_documents(doctype: str, filters: Optional[Dict] = None, 
                           fields: Optional[List[str]] = None, limit: int = 20) -> List[Dict]:
        ...
        tools.extend([
            get_document_tool, create_document_tool, update_document_tool, 
            delete_document_tool, search_documents, submit_document_tool, 
        ...
        if self.tools:
            ...
            for tool in self.tools:
                ...
                if hasattr(tool, 'description'):
                    ...
                    tool_descriptions.append(f"- code_interpreter: Execute Python code to analyze data, process files, and perform calculations")
                else:
...
7. For questions about invoice amounts, document content, or file information, ALWAYS use 'analyze_conversation_file' with an appropriate query."""
        ...
        model_settings = ModelSettings(
            temperature=self.bot_doc.temperature,
            top_p=self.bot_doc.top_p
        )
...
async def handle_ai_request_async(bot, message: str, channel_id: str, conversation_history: list = None, file_handler=None):
...
def handle_ai_request_sync(bot, message: str, channel_id: str, conversation_history: list = None, file_handler=None):
