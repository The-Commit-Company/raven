...
class ConversationFileHandler:
    ...
    def create_file_analysis_tool(self) -> Optional[FunctionTool]:
        """Create a tool to analyze files in current conversation"""
        if not self.conversation_files:
            return None
            
        def analyze_conversation_file(query: str, file_name: Optional[str] = None) -> dict:
            """
            ...
            try:
                ...
                if file_name:
                    files_to_analyze = {k: v for k, v in self.conversation_files.items() 
                                      if file_name.lower() in v['file_name'].lower()}
                
                if not files_to_analyze:
                    return {
                        "success": False,
                        "message": f"No files found matching '{file_name}'" if file_name else "No files in conversation"
                    }
                ...
                for msg_id, file_info in files_to_analyze.items():
                    ...
                    if file_type in ['xlsx', 'xls', 'csv']:
                        if content and not content.startswith("Error"):
                            ...
                            result["analysis"] = content if content.startswith("Error") else "Unable to read spreadsheet"
                        ...
                        result["content_preview"] = content[:1000] + "..." if len(content) > 1000 else content
                        result["analysis"] = f"File ready for analysis"
        ...
        tool = FunctionTool(
            ...
            params_json_schema={
                ...
                "properties": {
                    ...
                    "file_name": {
                        "type": "string",
                        "description": "Optional: specific file name to analyze"
                    }
                },
                "required": ["query"]
            },
