...
export const PreviousMessageBox = ({ previous_message_id, previous_message_content, onReplyingToMessageClose, channelData, channelMembers }: PreviousMessageBoxProps) => {
                ...
                <HStack w='full' p='2' rounded='md' justify="space-between">
                    ...
                    <Stack spacing={1}>
                        <HStack divider={<StackDivider />}>
                            <Text fontSize='sm' fontWeight={'semibold'}>{channelMembers?.[previous_message_content.owner]?.full_name ?? allUsers?.[previous_message_content.owner]?.full_name ?? previous_message_content.owner}</Text>
                            <HStack spacing={1}>
...
const PreviousMessageBoxInChat = ({ previous_message_id, channelData, channelMembers }: PreviousMessageBoxInChatProps) => {
    ...
    if (data) {
        return <LinkBox onClick={() => handleScrollToMessage(previous_message_id)} p='2' border={'1px'} borderColor={colorMode === 'light' ? 'gray.200' : 'gray.600'} rounded={'md'} _hover={{ cursor: 'pointer', boxShadow: 'sm', bgColor: colorMode === 'light' ? 'white' : 'black' }}>
            <Box pl='2' borderLeft={'2px'} borderLeftColor={colorMode === 'light' ? 'gray.600' : 'gray.600'}>
                <Stack spacing={1}>
                    <HStack>
                        <Text fontSize='xs' fontWeight={'semibold'} color='blue.500'>{channelMembers?.[data.owner]?.full_name ?? allUsers?.[data.owner]?.full_name ?? data.owner}</Text>
                        <HStack spacing={1}>
