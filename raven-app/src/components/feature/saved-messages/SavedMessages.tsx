import { Text, Stack, useToast, Tooltip, Button, useDisclosure } from "@chakra-ui/react"
...
export const SavedMessages = () => {
    ...
    const { data, error, mutate } = useFrappeGetCall<{ message: SavedMessage[] }>("raven.raven_messaging.doctype.raven_message.raven_message.get_saved_messages", undefined, undefined, {
        revalidateOnFocus: false
    })
    ...
    return (
        <>
            ...
            <Stack h='calc(100vh)' justify={'flex-start'} p={4} overflow='scroll' pt='20'>
                {data?.message?.map(({ name, text, owner, creation, channel_id, file }: SavedMessage) => {
                    ...
                    return (
                        <MessageBox isArchived={isArchived} channelName={channelName} messageName={name} channelID={channel_id} creation={creation} owner={owner} messageText={text} full_name={full_name} user_image={user_image} file={file} handleScrollToMessage={handleScrollToMessage} />
                    )
                }
                )}
            </Stack>
