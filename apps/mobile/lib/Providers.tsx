...
import i18n, { __ } from '@lib/i18n';

const Providers = (props: PropsWithChildren) => {
    // Listen to language changes and force re-render of all children
...
const { users, enabledUsers, isLoading, error, hasData, mutate } = useUserListProvider()
    ...
    if (!hasData && error) {
        return <View className='bg-card px-2 gap-2 h-screen' style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <ErrorBanner error={error} heading={__("There was an error while loading the user list.")} />
            <View className='flex w-full gap-2 pt-8'>
                <TouchableOpacity onPress={() => mutate()}
                    ...
                    <Text className="font-medium text-foreground">{__("Reload App")}</Text>
...
const channelListContextData = useChannelListProvider()
    ...
    if (!channelListContextData.hasData && channelListContextData.error) {
        return <View className='bg-card px-2 gap-2 h-screen' style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <ErrorBanner error={channelListContextData.error} heading={__("There was an error while loading the channel list.")} />
            <View className='flex w-full gap-2 pt-8'>
