...
/**
 * Global notifications realtime, mounted once in AppListeners. The list-affecting
 * events carry no row data, so a new mention/reaction triggers a page-0 reconcile
 * of the single merged window (when seeded). The read events flip is_read in place.
 * Every event also revalidates the shared unread-count key, keeping the sidebar +
 * page badge live even when the Notifications page is unmounted.
 */
export const useNotificationsRealtime = () => {
    ...
    const onNewNotification = () => {
        if (notificationListStore.isLoaded()) reconcileFirstPage(client)
        globalMutate(UNREAD_COUNT_KEY)
    }

    useFrappeEventListener("raven_mention", onNewNotification)
    useFrappeEventListener("raven_reaction_notification", onNewNotification)
