...
export const BatchMessageItem = ({
    ...
    // one-off rows. pdf + non-previewable both go to the pill grid.
    const images = block.messages.filter((message) => kindOf(message) === "image")
    const videos = block.messages.filter((message) => kindOf(message) === "video")
    const audios = block.messages.filter((message) => kindOf(message) === "audio")
    const docs = block.messages.filter((message) => {
