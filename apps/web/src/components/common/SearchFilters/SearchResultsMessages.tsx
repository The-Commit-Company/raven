        ...
        <div
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between gap-2 mb-0.5">
                    <div className="text-sm text-ink-gray-8 line-clamp-3 w-[calc(100vw-20rem)]">
                        <MarkdownRenderer content={message.content} />
                    </div>
                    <span className="text-xs text-ink-gray-4 shrink-0">{formatDate(message.creation, "D MMMM YYYY h:mm A")}</span>
                </div>
        ...
        <div className="space-y-2">
            ...
            <ScrollArea className="flex-1">
                {isLoading || !results ? <MessageListSkeleton /> :
                    results.length === 0 ? <div className="text-sm text-ink-gray-4 text-center py-8">{_("No messages found.")}</div> :
